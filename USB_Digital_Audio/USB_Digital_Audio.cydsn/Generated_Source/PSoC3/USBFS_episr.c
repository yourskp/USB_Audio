...
/* `#END` */
    ...
    ******************************************************************************/
    CY_ISR(USBFS_EP_1_ISR)
    {
		...
		ProcessAudioOut();
			...
			!inPlaying &&
			#endif
			IsUSBMidiActive() && !(USBFS_midiInPointer%USBFS_EVENT_LENGTH))
			{
    ...
    *******************************************************************************/
    CY_ISR(USBFS_EP_7_ISR)
    {
		...
		if(CY_GET_REG8(USBFS_SIE_EP7_CR0_PTR) & ACKED_TRANSACTION_MASK)
		{
			CY_SET_REG8(USBFS_SIE_EP7_CR0_PTR, CY_GET_REG8(USBFS_SIE_EP7_CR0_PTR) & (~ACKED_TRANSACTION_MASK));
		...
		{
			DisableNAKBulkIN(MIDI_IN_ENDPOINT);
		}
		/* If MIDI endpoint is either ACK'ed or NAK'ed then MIDI is active */
		midiEpPolled = TRUE;
