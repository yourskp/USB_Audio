/*******************************************************************************
* File Name: IR_Reference_Clock.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: IR_Reference_Clock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IR_Reference_Clock_Start(void) 
{
    /* Set the bit to enable the clock. */
    IR_Reference_Clock_CLKEN |= IR_Reference_Clock_CLKEN_MASK;
	IR_Reference_Clock_CLKSTBY |= IR_Reference_Clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: IR_Reference_Clock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IR_Reference_Clock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    IR_Reference_Clock_CLKEN &= (uint8)(~IR_Reference_Clock_CLKEN_MASK);
	IR_Reference_Clock_CLKSTBY &= (uint8)(~IR_Reference_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: IR_Reference_Clock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IR_Reference_Clock_StopBlock(void) 
{
    if ((IR_Reference_Clock_CLKEN & IR_Reference_Clock_CLKEN_MASK) != 0u)
    {
...
#if defined(IR_Reference_Clock__CFG3)
        CLK_DIST_AMASK = IR_Reference_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = IR_Reference_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* IR_Reference_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(IR_Reference_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(IR_Reference_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: IR_Reference_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IR_Reference_Clock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        IR_Reference_Clock_CLKSTBY &= (uint8)(~IR_Reference_Clock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: IR_Reference_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IR_Reference_Clock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = IR_Reference_Clock_GetSourceRegister();
    uint16 oldDivider = IR_Reference_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = IR_Reference_Clock_CLKEN & IR_Reference_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(IR_Reference_Clock_DIV_PTR, clkDivider);
                IR_Reference_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                IR_Reference_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(IR_Reference_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((IR_Reference_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(IR_Reference_Clock_DIV_PTR, clkDivider);
				IR_Reference_Clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: IR_Reference_Clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 IR_Reference_Clock_GetDividerRegister(void) 
{
    return CY_GET_REG16(IR_Reference_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: IR_Reference_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IR_Reference_Clock_SetModeRegister(uint8 modeBitMask) 
{
    IR_Reference_Clock_MOD_SRC |= modeBitMask & (uint8)IR_Reference_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: IR_Reference_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IR_Reference_Clock_ClearModeRegister(uint8 modeBitMask) 
{
    IR_Reference_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(IR_Reference_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: IR_Reference_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 IR_Reference_Clock_GetModeRegister(void) 
{
    return IR_Reference_Clock_MOD_SRC & (uint8)(IR_Reference_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: IR_Reference_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IR_Reference_Clock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = IR_Reference_Clock_GetDividerRegister();
    uint8 oldSrc = IR_Reference_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        IR_Reference_Clock_MOD_SRC |= CYCLK_SSS;
        IR_Reference_Clock_MOD_SRC =
            (IR_Reference_Clock_MOD_SRC & (uint8)(~IR_Reference_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: IR_Reference_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 IR_Reference_Clock_GetSourceRegister(void) 
{
    return IR_Reference_Clock_MOD_SRC & IR_Reference_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: IR_Reference_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void IR_Reference_Clock_SetPhaseRegister(uint8 clkPhase) 
{
    IR_Reference_Clock_PHASE = clkPhase & IR_Reference_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: IR_Reference_Clock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 IR_Reference_Clock_GetPhaseRegister(void) 
{
    return IR_Reference_Clock_PHASE & IR_Reference_Clock_PHASE_MASK;
}
